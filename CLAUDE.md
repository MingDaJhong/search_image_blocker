...
1. **Content script** (`entrypoints/content/index.ts`) — injected into 16 Google TLDs (`google.com`, `google.com.tw`, `google.co.jp`, …) at `document_start`. Imports from `@/composables/blockList` (the pure layer) — never from `useBlockList`, to keep Vue runtime out of the content bundle. The single `applyState(settings)` function is the source of truth for what's currently active; it runs once at boot and again on every `browser.storage.onChanged` event so settings changes apply live without a page reload. It coordinates two parallel tracks:
   ...
   - **Page-level blocking (CSS)**: phase 1 inject a temporary "hide suspicious blocks" `<style>` (`sib-initial-hide`); phase 2 `await loadSettings()` and call `shouldBlock(urlQuery, settings)`; if true, swap in the real selector list (`sib-block-style`), if false remove the temporary one. Selectors are produced by `collectBlockSelectors(settings)` (a pure helper) and concatenated by `buildBlockCSS()`. The CSS targets Google's DOM (`g-scrolling-carousel`, `video-voyager`, `div[data-attrid*="..."]`) — **Google rotates these regularly**, expect to update them when blocking breaks.
   - **Dev-only selector audit**: 2 seconds after `applyState`, when `import.meta.env.DEV`, `devSelectorAudit()` runs `querySelectorAll` against every active selector and `console.warn`s if all of them returned 0 elements (likely sign of stale selectors). Stripped from production builds by Vite.
...
3. **Shared logic** — split across two files for content/popup boundary clarity:
   ...
   - `useBlockList()` exposes: `settings`, `loaded`, `saveError`, `addKeyword`, `removeKeyword`, `addCategory`, `addCategoryFromPreset`, `removeCategory`, `setCatLabel`, `addCatKeyword`, `removeCatKeyword`. Mutators that take user-typed strings (`addKeyword`, `addCatKeyword`) return `AddKeywordResult` (`'added' | 'duplicate' | 'empty' | 'too_long'`) so the UI can render inline validation feedback. The same length caps (`MAX_KEYWORD_LEN` / `MAX_LABEL_LEN`) are also enforced inside `loadSettings` / `parseImport` filters, so corrupted storage or a malicious import can't smuggle in oversized strings.
...
The manifest is generated by WXT from `wxt.config.ts` — edit it there, not in `.output/`. Current permissions: `storage` only, with host permissions covering 16 Google TLDs (`com`, `com.tw`, `com.hk`, `co.jp`, `co.kr`, `com.sg`, `co.uk`, `com.au`, `ca`, `co.in`, `de`, `fr`, `es`, `it`, `com.br`, `com.mx`). The list lives in two places that must stay in sync: `host_permissions` in `wxt.config.ts` (uses a `GOOGLE_TLDS.map(...)` because Node-side config can compute), **and** the `matches: [...]` literal in `entrypoints/content/index.ts` (must be a string array literal — WXT statically analyzes it for the manifest at build time and won't evaluate `.map()` calls).
